...
//    """.trimIndent()
    ...
    val result = lines.sumOf { line ->
        ...
        "${firstDigit.digitToInt()}${lastDigit.digitToInt()}".toInt()
    }

    println(result)
}

// 47857 is not right answer!
// 49247 is not right answer!
// 49209 is not right answer!
// 54961 is not right
fun day1part2() {
    val input = inputDay1

//    val input = "6hznsq5qjcjsvgnl"
//    val input = "twohznsqfiveqjcjsvgnl"
//val input = "nineseven2sixnineqd"
//    val input = """
//        two1nine
//        eightwothree
//        abcone2threexyz
...
//        "zero" to 0,
    ...
    val fakeNumbers = map.keys

    val result = lines.sumOf { line ->
        val closestNumber = if (fakeNumbers.any { line.contains(it) }) {
            fakeNumbers.minBy { number ->
        ...
        val closestIndex = closestNumber?.let { line.indexOf(it) } ?: Int.MAX_VALUE
        var firstDigitIndex = line.indexOfFirst { it.isDigit() }
        if (firstDigitIndex == -1) {
            firstDigitIndex = Int.MAX_VALUE
        }

        val firstValue = if (closestIndex <= firstDigitIndex) {
            map[closestNumber]!!
        } else {
        ...
        val farthestNumber = fakeNumbers.maxBy { number ->
            line.lastIndexOf(number)
        }
        val farthestIndex = line.lastIndexOf(farthestNumber)
        val lastDigitIndex = line.indexOfLast { it.isDigit() }

        val lastValue = if (farthestIndex >= lastDigitIndex) {
            map[farthestNumber]!!
        } else {
...
// 6hznsq5qjcjsvgnl  is wrong
// Check what happens when there are no numbers as text! Or vce versa!
